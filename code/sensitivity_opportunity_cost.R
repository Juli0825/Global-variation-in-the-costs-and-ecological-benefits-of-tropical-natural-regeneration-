# Cost of natural regeneration sensitivity analysis
# This script examines the effect of excluding opportunity costs from natural regeneration cost calculations
...
#----------------------------------------------------------------------
# Divide sensitivity costs into 3 quantile-based categories (Low, Medium, High)
pnr_df <- pnr_df %>%
  mutate(
    nrcost_category = ntile(sens_costs, 3),  # Use sens_costs instead of nrcosts
...
sens_raster <- calc(sens_raster, fun = function(x) { as.integer(x) })

# Save the raster
writeRaster(sens_raster, filename = "results/sensitivity_no_oppcost.tif", 
...
res_x <- res(sens_raster)[1]  # Resolution in X direction (meters)
res_y <- res(sens_raster)[2]  # Resolution in Y direction (meters)
...
for (x in 1:nrow(gadm_moll)) {
  ...
  # Initialize a vector to store the area for each category (1 to 12)
  ...
  for (i in 1:12) {
    ...
    cell_count_list <- exact_extract(sens_raster, country_polygon, function(values, coverage_fraction) {
  ...
  sens_results <- rbind(sens_results, new_row)
}

# Save the results to an Excel file
write_xlsx(sens_results, "results/sensitivity_no_oppcost_statistics.xlsx")
